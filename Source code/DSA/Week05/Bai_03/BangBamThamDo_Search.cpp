...
// nprob la so lan bam lai
//###INSERT CODE HERE -
    if (ht.n + 1 >= ht.M * LOAD)
        return -1;
    int idxHash = maso % ht.M;
    int initialIdx = idxHash; // luu giu vi tri hien tai: initial co nghia la ban dau
    nprob = 0;
    while (ht.table[idxHash].Maso != maso && ht.table[idxHash].Maso != EMPTY)
    {
        nprob++;
        idxHash = (initialIdx + nprob * nprob) % ht.M;
        if (nprob >= ht.M)
            return 1;
    }
    if (ht.table[idxHash].Maso == maso)
        return nprob; // tra ve so lan tham do
    return -1;
}